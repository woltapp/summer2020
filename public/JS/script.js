(function () {
  ...
  searchRestaurant.onsubmit = function search(event) {
    ...
    ascAlphabetSortingBtn.classList.remove('active');
    descAlphabetSortingBtn.classList.remove('active');
    let numberOfPage = 1;
    let requestUrl = '';

    function createListOfRestaurants(elem) {
      if (elem === undefined) {
    ...
    ascAlphabetSortingBtn.onclick = function () {
      event.preventDefault();
      ascAlphabetSortingBtn.classList.add('active');
      descAlphabetSortingBtn.classList.remove('active');
      createListOfRestaurants('&sort=asc');
      window.history.replaceState({}, '', '/asc');
    };

    descAlphabetSortingBtn.onclick = function () {
      event.preventDefault();
      descAlphabetSortingBtn.classList.add('active');
      ascAlphabetSortingBtn.classList.remove('active');
      createListOfRestaurants('&sort=desc');
    ...
    pagination.onclick = function (e) {
      ...
      if (ascAlphabetSortingBtn.classList.contains('active')) {
        createListOfRestaurants('&sort=asc');
      } else if (descAlphabetSortingBtn.classList.contains('active')) {
        createListOfRestaurants('&sort=desc');
      } else {
        createListOfRestaurants();
      }
